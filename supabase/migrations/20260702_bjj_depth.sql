...
-- Final: also showcase roll/sub/technique/partner depth on demo sessions.
create or replace function public.reseed_demo_full()
returns void language plpgsql security definer set search_path to 'public' as $$
declare d uuid := '581543c1-a23a-4f95-a7ed-c98fb285ece7';
begin
  ...
  insert into bjj_promotions(user_id, belt, stripes, promoted_at) values
    ...
    (d,'purple',0, now()-interval '300 days'),
    (d,'purple',2, now()-interval '60 days');
  update bjj_sessions set rounds=6, subs_for=3, subs_against=2,
    techniques=array['Guard Passing','Back Takes'], partners=array['Marcus','Dev']
    where user_id=d and kind='open_mat' and notes like 'Six rolls%';
  update bjj_sessions set rounds=4, subs_for=2, subs_against=1,
    techniques=array['Sweeps','Half Guard'], partners=array['Coach Ana']
    where user_id=d and kind='class' and notes like 'Worked half-guard%';
end $$;
